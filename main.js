...
window.addEventListener('scroll', () => {
  ...
  let anyActive = false;
  let navbarStyleSet = false;
  document.querySelectorAll("section[id]").forEach((section) => {
    const sectionTop = section.offsetTop - 139; // Offset for navbar
    const sectionHeight = section.offsetHeight;
    const sectionId = section.getAttribute("id");
    console.log(`Section: ${sectionId}, Top: ${sectionTop}, Height: ${sectionHeight}`);
    if (scrollY > sectionTop + 139 && scrollY < sectionTop + sectionHeight + 140) {
      const style = section.dataset.navbar;
      ...
      } else if (style === "transparent-blur") {
        navbar.classList.add("bg-transparent", "blur-navbar");
      }
  ...
  // Fallback: if no match (like at top), clear background
  if (!navbarStyleSet) {
    navbar.classList.remove(
      "bg-dark",
      "bg-light",
